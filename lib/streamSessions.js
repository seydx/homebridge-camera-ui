...
class streamSessions {
  ...
  closeSession(camera, plugin){
    ...
    plugin 
      ? Logger.debug('Session: Closing FFmpeg process', camera) 
      : Logger.ui.debug('Session: Closing FFmpeg process', camera);
    
    if(this.sessions[camera].activeStreams === 0){
      
      plugin 
        ? Logger.warn('Session: There is an abnormality with the session handler. Please check your active FFMpeg streams. Otherwise please restart camera.ui.', camera) 
        : Logger.ui.warn('Session: There is an abnormality with the session handler. Please check your active FFMpeg streams. Otherwise please restart camera.ui.', camera);
    ...
    plugin 
      ? Logger.debug('Currently active stream: ' + JSON.stringify(this.sessions), 'Session') 
      : Logger.ui.debug('Currently active stream: ' + JSON.stringify(this.sessions), 'Session');
